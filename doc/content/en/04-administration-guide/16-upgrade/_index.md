...
In order for the existing SGClusters, SGShardedClusters and SGDistributedLogs to make use of all the new functionalities and bugfixes available in the updated operator version, an [SGDbOps security upgrade]({{% relref "06-crd-reference/08-sgdbops#sgdbopsspecsecurityupgrade" %}}) for all existing SGCluster and SGDistributedLogs or an [SGShardedDbOps security upgrade]({{% relref "06-crd-reference/14-sgshardeddbops#sgshardeddbopsspecsecurityupgrade" %}}) for all existing SGShardedCluster has to be performed. Since this operation require restarting the primary Postgres instance it should be performed during a maintenance window.
