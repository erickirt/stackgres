...
public class StackGresShardedDbOpsSpec {
  ...
  @ValidEnum(enumClass = ShardedDbOpsOperationAllowed.class, allowNulls = false,
      message = "op must be one of resharding, restart, securityUpgrade,"
          + " majorVersionUpgrade or minorVersionUpgrade")
  private String op;
  ...
  public int hashCode() {
    return Objects.hash(majorVersionUpgrade, maxRetries, minorVersionUpgrade, op, resharding,
        restart, retryDelay, retryLimit, retryMaxDelay, runAt, scheduling, securityUpgrade,
        sgShardedCluster, timeout);
  }
