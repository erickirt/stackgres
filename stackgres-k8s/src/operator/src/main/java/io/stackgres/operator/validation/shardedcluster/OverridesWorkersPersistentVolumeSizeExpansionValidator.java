...
public class OverridesWorkersPersistentVolumeSizeExpansionValidator
  ...
  @Inject
  public OverridesWorkersPersistentVolumeSizeExpansionValidator(
  ...
  class OverrideShardPersistentVolumeSizeExpansionValidator
    ...
    protected @NotNull String getVolumeSize(StackGresShardedCluster cluster) {
      return Optional.of(cluster.getSpec().getWorkers())
          ...
          .orElse(cluster.getSpec().getWorkers().getPods().getPersistentVolume().getSize());
    }
    ...
    protected Optional<String> getStorageClass(StackGresShardedCluster cluster) {
      return Optional.of(cluster)
          ...
          .flatMap(override -> Optional.of(override)
              ...
              .stream())
          .findFirst()
          .or(() -> Optional.of(cluster.getSpec().getWorkers().getPods().getPersistentVolume()))
    ...
    protected List<StackGresCluster> getClusters(StackGresShardedCluster resource) {
      return clusterScanner.getResourcesWithLabels(
          resource.getMetadata().getNamespace(), labelFactory.workersLabels(resource));
    }
