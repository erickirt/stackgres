...
public class CitusShardedClusterUpdateWorkersSecret implements ResourceGenerator<StackGresShardedClusterContext> {

  final LabelFactoryForShardedCluster labelFactory;

  @Inject
  public CitusShardedClusterUpdateWorkersSecret(LabelFactoryForShardedCluster labelFactory) {
  ...
  public Stream<HasMetadata> generateResource(StackGresShardedClusterContext context) {
    return Seq.of(Boolean.TRUE)
        ...
        .<HasMetadata>map(ignore -> StackGresShardedClusterForCitusUtil
            .getUpdateWorkersSecret(context))
        .map(config -> {
