...
public class ShardedClusterWorkersPgBouncerDefaultPoolingConfig
  ...
  @Inject
  public ShardedClusterWorkersPgBouncerDefaultPoolingConfig(
  ...
  public Stream<HasMetadata> generateResource(StackGresShardedClusterContext context) {
    return Stream
        .of(Optional.of(context.getSource().getSpec().getWorkers().getPods())
  ...
  private StackGresPoolingConfig getDefaultConfig(StackGresShardedCluster cluster) {
    return new StackGresPoolingConfigBuilder()
        ...
        .withName(cluster.getSpec().getWorkers().getConfigurations().getSgPoolingConfig())
