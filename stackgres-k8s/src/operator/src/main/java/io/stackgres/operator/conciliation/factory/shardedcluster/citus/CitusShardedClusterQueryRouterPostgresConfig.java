...
public class CitusShardedClusterQueryRouterPostgresConfig implements ResourceGenerator<StackGresShardedClusterContext> {
  ...
  @Inject
  public CitusShardedClusterQueryRouterPostgresConfig(
  ...
  public Stream<HasMetadata> generateResource(StackGresShardedClusterContext context) {
    return Seq.of(Boolean.TRUE)
        ...
        .flatMap(ignore -> context.getQueryRoutersPostgresConfigs().stream())
        .<HasMetadata>map(queryRoutersPostgresConfig -> StackGresShardedClusterForCitusUtil
            .getQueryRouterPostgresConfig(
                context.getSource(),
                queryRoutersPostgresConfig.v1,
                queryRoutersPostgresConfig.v2
                .orElse(defaultPostgresConfigFactory.buildResource(context.getResource()))))
        .map(config -> {
          config.getMetadata().setLabels(labelFactory.queryRoutersLabels(context.getSource()));
          return config;
        });
  }
