...
public class ShardedClusterWorkersPoolingConfigContextAppender
  ...
  public void appendContext(StackGresShardedCluster cluster, Builder contextBuilder) {
    final Optional<StackGresPoolingConfig> workersPoolingConfig = Optional
        .ofNullable(cluster.getSpec().getWorkers().getConfigurations().getSgPoolingConfig())
        .flatMap(poolingConfigName -> poolingConfigFinder
            .findByNameAndNamespace(
                poolingConfigName,
                cluster.getMetadata().getNamespace()));
    if (!cluster.getSpec().getWorkers().getConfigurations().getSgPoolingConfig()
        .equals(defaultPoolingConfigFactory.getDefaultResourceName(cluster))
        && !Optional.ofNullable(cluster.getSpec().getWorkers()
    ...
    contextBuilder.workersPoolingConfig(workersPoolingConfig);
  }
