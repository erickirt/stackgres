...
class DefaultWorkersProfileConfigMutatorTest
    extends AbstractDefaultResourceMutatorTest<StackGresInstanceProfile, HasMetadata, StackGresShardedCluster,
        StackGresShardedClusterReview, DefaultWorkersProfileMutator> {
  ...
  @Override
  protected DefaultWorkersProfileMutator getDefaultConfigMutator() {
    var resourceFactory = new DefaultProfileFactory();
    var mutator = new DefaultWorkersProfileMutator(
        resourceFactory);
    return mutator;
  }
  ...
  protected void checkConfigurationIsSet(StackGresShardedCluster newResource) {
    ...
    assertNotNull(newResource.getSpec().getWorkers().getSgInstanceProfile());
  }
