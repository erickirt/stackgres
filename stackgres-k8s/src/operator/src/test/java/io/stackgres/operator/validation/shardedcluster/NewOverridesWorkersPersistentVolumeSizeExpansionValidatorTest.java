...
class NewOverridesWorkersPersistentVolumeSizeExpansionValidatorTest extends
  ...
  protected Validator<StackGresShardedClusterReview> getValidator() {
    return new OverridesWorkersPersistentVolumeSizeExpansionValidator(
  ...
  protected void setVolumeSize(StackGresShardedCluster cluster, String size) {
    if (cluster == review.getRequest().getOldObject()) {
  ...
  protected String getStorageClassName(StackGresShardedCluster cluster) {
    if (cluster == review.getRequest().getOldObject()) {
      return cluster.getSpec().getWorkers()
