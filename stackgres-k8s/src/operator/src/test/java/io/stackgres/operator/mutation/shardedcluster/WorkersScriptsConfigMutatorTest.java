...
class WorkersScriptsConfigMutatorTest {

  protected static final ObjectMapper JSON_MAPPER = new ObjectMapper();

  private WorkersScriptsConfigMutator mutator;

  @BeforeEach
  void setUp() throws Exception {
    mutator = new WorkersScriptsConfigMutator();
  }
  ...
  void createClusterWithouIds_shouldAddThem() {
    ...
    review.getRequest().getObject().getSpec().getWorkers().getManagedSql().getScripts().stream()
  ...
  void updateClusterAddingAnEntry_shouldSetId() {
    ...
    review.getRequest().getObject().getSpec().getWorkers().getManagedSql().getScripts()
        .add(1, new StackGresClusterManagedScriptEntry());

    StackGresShardedCluster expected = JsonUtil.copy(review.getRequest().getObject());
    expected.getSpec().getWorkers().getManagedSql().getScripts().get(1).setId(13);
